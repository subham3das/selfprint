...
interface SidebarProps {
  ...
  printerStatus?: PrinterStatusInfo | null;
    ...
    <div className="flex flex-col h-full bg-[#0A0F1D] text-slate-200 w-64 border-r border-slate-800/80 select-none">
          ...
          {effectivePrinterConfigured ? (
            <>
              <div className="flex items-center gap-2">
                ...
                >
                  {isPaused ? 'Paused' : printerStatus?.isOnline ? 'Online' : 'Offline'}
                </span>
              </div>
              <p className="text-xs font-semibold text-slate-100 mt-1 truncate">
                {printerStatus?.name || 'Configured Printer'}
              </p>

              <button
                onClick={onTogglePause}
                className={`w-full mt-3 py-2 px-3 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-all duration-150 cursor-pointer ${
                  isPaused
            ...
            <>
              ...
              <button
                ...
                className="w-full mt-3 py-2 px-3 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 bg-purple-600/20 border border-purple-500/30 text-purple-300 hover:bg-purple-600/30 transition-all duration-150 cursor-pointer"
              >
      ...
      <div className="p-3 border-t border-slate-800/80 space-y-1 relative">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-slate-500 font-mono mt-0.5">
              Store ID: {storeInfo.storeCode || 'SP-0000'}
            </p>
          </div>
          <ChevronDown
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 text-xs font-medium transition-colors cursor-pointer"
        >
          ...
          {showProfileMenu && (
            ...
            >
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-slate-300 hover:text-white hover:bg-slate-800 transition-colors cursor-pointer"
              >
