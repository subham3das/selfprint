    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div className="p-3 rounded-xl border border-slate-100 bg-white space-y-1">
                ...
                <p className="font-bold text-slate-800 text-sm">
                  {(job as any).printerName || 'Main Store Printer'}
                </p>
              </div>
            </div>
          </div>
          ...
          {/* Footer */}
          <div className="flex items-center justify-between gap-2.5 p-4 border-t border-slate-100 bg-slate-50/50">
            <div>
              {Boolean((job as any).isVirtual || (job as any).printerName?.toLowerCase().includes('virtual') || (job as any).printerName?.toLowerCase().includes('pdf')) && (
                <span className="inline-flex items-center gap-1 text-[11px] font-bold text-purple-700 bg-purple-50 border border-purple-200 px-2.5 py-1 rounded-lg">
                  🧪 Test Print &bull; Saved to Documents/SelfPrint/TestPrints/
                </span>
              )}
            </div>
            <div className="flex items-center gap-2">
              {job.status === 'Completed' && (
                <button
                  type="button"
                  onClick={() => {
                    const filePath = `receipt-${job.id || job.jobCode}.pdf`;
                    alert(`Virtual PDF Generated:\n\nLocation: Documents\\SelfPrint\\TestPrints\\${filePath}\n\nYou can open and inspect this file on your local machine.`);
                  }}
                  className="px-3 py-2 rounded-xl border border-purple-200 bg-purple-50 text-purple-700 hover:bg-purple-100 text-xs font-semibold transition-colors"
                >
                  View Generated PDF
                </button>
              )}
