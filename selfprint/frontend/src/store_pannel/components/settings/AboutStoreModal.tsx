...
import { X, Info, CheckCircle2, ShieldCheck, Database, Server } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { SystemInfo } from '../../services/storeSettings.service';
            ...
            <div className="p-3.5 rounded-xl border border-slate-100 space-y-2.5">
              <div className="flex items-center justify-between text-slate-600">
                ...
                <span className="font-bold text-slate-800 font-mono">
                  {systemInfo?.backendVersion || 'v0.1.0-prod'} ({systemInfo?.buildNumber || 'SP-2026.08.30'})
                </span>
              </div>
              <div className="flex items-center justify-between text-slate-600">
                <span>API Gateway</span>
                <span className="font-medium text-slate-800">
                  {systemInfo?.apiVersion || 'v1.4'} (REST / WebSocket)
                </span>
              </div>
              <div className="flex items-center justify-between text-slate-600">
                ...
                <span className="font-medium text-indigo-600 flex items-center gap-1">
                  ...
                  {systemInfo?.databaseStatus || 'Connected (MongoDB Atlas)'}
                </span>
              </div>
              <div className="flex items-center justify-between text-slate-600">
              ...
              <div className="flex items-center justify-between text-slate-600">
                ...
                <span className="font-medium text-emerald-600 flex items-center gap-1">
                  ...
                  {systemInfo?.nodeVersion || 'v20.x'} ({systemInfo?.environment || 'production'})
                </span>
              </div>
              <div className="flex items-center justify-between text-slate-600">
                <span>Uptime</span>
                <span className="font-medium text-slate-800">
                  {systemInfo ? `${Math.floor(systemInfo.uptimeSeconds / 60)} mins` : 'Live'}
                </span>
              </div>
            </div>
          </div>
