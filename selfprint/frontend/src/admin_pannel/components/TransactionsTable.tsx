...
export const TransactionsTable: React.FC<TransactionsTableProps> = ({
  ...
  const renderPaymentMethod = (method: PaymentMethod | string) => {
    switch (method) {
      case 'UPI':
        return (
      ...
      default:
        return <span className="text-slate-600 text-xs">{method || 'UPI'}</span>;
    }
  };
            ...
            {isLoading ? (
              Array.from({ length: 5 }).map((_, idx) => (
                <tr key={`txn-skel-${idx}`} className="animate-pulse">
                  ...
                  <td className="py-3.5 px-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="w-28 h-2 bg-slate-100 rounded mt-1" />
                      </div>
                    </div>
                  </td>
                  <td className="py-3.5 px-3 text-center">
                    <div className="w-16 h-4 bg-slate-100 rounded-full mx-auto" />
                  </td>
                  <td className="py-3.5 px-3 text-center">
                    <div className="w-6 h-3 bg-slate-100 rounded mx-auto" />
                  </td>
                  ...
                  <td className="py-3.5 px-3 text-center">
                    <div className="w-12 h-3 bg-slate-100 rounded mx-auto" />
                  </td>
                  <td className="py-3.5 px-3 text-center">
                    <div className="w-16 h-4 bg-slate-100 rounded-full mx-auto" />
                  </td>
                ...
                >
                  ...
                  <td className="py-3.5 px-3">
                    <div className="flex items-center gap-2.5">
                      <div
                        className={`w-7 h-7 rounded-full ${txn.storeLogoBg || 'bg-slate-900 text-white'} flex items-center justify-center font-black text-[11px] shrink-0 shadow-2xs`}
                      >
                        {txn.storeLogoText || 'SP'}
                      </div>
                  ...
                  <td className="py-3.5 px-3">
                    <div className="flex items-center gap-2.5">
                      <img
                        src={
                          txn.customerAvatar ||
                          `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(txn.customerName || 'U')}`
                        }
