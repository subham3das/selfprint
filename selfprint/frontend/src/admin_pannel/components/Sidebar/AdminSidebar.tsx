...
export const AdminSidebar: React.FC<AdminSidebarProps> = ({
  ...
  const { can, isSuperAdmin, role, user } = usePermission();
  ...
  // Dynamically filter menu based on O(1) permission evaluation
  const visibleNavItems = useMemo(() => {
    return NAV_ITEMS.filter((item) => can(item.id, 'view'));
  }, [can]);
        ...
        <div className="flex items-center gap-3 px-1 py-1">
          ...
          {!isCollapsed && (
            <div className="min-w-0">
              ...
              <p className="text-purple-400 text-xs font-semibold mt-1 truncate">
                {roleLabel}
              </p>
            </div>
          )}
        ...
        {!isCollapsed && (
          <div className="relative bg-[#13162C] border border-indigo-900/40 rounded-2xl p-3.5 flex flex-col items-center text-center shadow-inner overflow-hidden">
            ...
            <p className="text-[10px] text-slate-400 mt-1 leading-tight max-w-[170px]">
              {user?.displayName || user?.name || 'Administrator'} • {user?.email || 'admin@selfprint.com'}
            </p>
          </div>
        )}
      </div>
