...
export const SystemOverviewCard: React.FC<SystemOverviewCardProps> = ({ overview, onTriggerBackup }) => {
  return (
        ...
        <div className="pb-3 border-b border-slate-100 mb-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Live runtime telemetry and platform health.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3 text-xs">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono font-bold text-slate-900">
              {overview.platformVersion || 'v2.4.0-prod'}
            </span>
          </div>
          ...
          <div className="space-y-1 pt-0.5">
            <div className="flex items-center justify-between text-slate-600">
              ...
              <span className="font-mono font-bold text-slate-900">
                {overview.storageUsedPercent || 28}%
              </span>
            </div>
            <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
              <div
                className="bg-indigo-600 h-full rounded-full transition-all duration-500"
                style={{ width: `${overview.storageUsedPercent || 28}%` }}
              />
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono font-bold text-slate-900">
              {(overview.totalStores || 0).toLocaleString()}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-slate-600">
              ...
              <span className="font-medium">Fleet Printers</span>
            </div>
            <span className="font-mono font-bold text-slate-900">
              {(overview.totalPrinters || 0).toLocaleString()}
            </span>
          </div>
        </div>
      </div>

      {/* Snapshot Backup Button */}
      {onTriggerBackup && (
        <div className="pt-3 mt-3 border-t border-slate-100">
          <button
            type="button"
            onClick={onTriggerBackup}
            className="w-full py-2 bg-slate-50 hover:bg-indigo-50 border border-slate-200/80 hover:border-indigo-200 text-slate-700 hover:text-indigo-600 font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer shadow-2xs"
          >
            <DownloadCloud className="w-3.5 h-3.5" />
            <span>Create Database Snapshot</span>
          </button>
        </div>
      )}
    </div>
