...
export const AnalyticsStatsCards: React.FC<AnalyticsStatsCardsProps> = ({ stats, isLoading }) => {
  if (isLoading) {
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-3.5">
        {Array.from({ length: 6 }).map((_, idx) => (
          <div
            key={`anl-stat-skel-${idx}`}
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-3.5">
      {cards.map((c, idx) => {
        ...
        const svgW = 120;
        const svgH = 26;
        const pts = c.sparkline && c.sparkline.length > 0 ? c.sparkline : [0, 0, 0, 0];
