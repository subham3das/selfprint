...
├── backend/
│   ├── src/
│   │   ├── config/         # Environment variables, Cloudinary & MongoDB configuration
│   │   ├── constants/      # Status codes, enum constants, roles, permissions
...
│   │   ├── models/         # Mongoose schemas and models (users, stores, printers, print_jobs, etc.)
│   │   ├── modules/        # Feature-first modular subsystems:
│   │   │   ├── auth/           # Authentication, JWT, Refresh Tokens, RBAC
│   │   │   ├── super-admin/    # Super Admin system settings & platform controls
...
## 9. Authentication & Store Onboarding Architecture

### 9.1 Store Partner Onboarding Flow
1. **Wizard Form**: 4-step wizard on `/store/onboarding` collecting store identity, owner info, shop coordinates, Cloudinary image upload, and bank settlement details.
2. **Transactional Database Write**:
   - `stores`: Creates verified merchant profile with unique `storeCode` (e.g. `SP-1001`) and bcrypt-hashed password.
   - `store_bank_accounts`: Creates payout account mapping.
   - `store_settings`: Initializes default page pricing (B&W ₹2, Color ₹10) and business hours.
   - `qr_links`: Generates unique linked QR standee token.
3. **Cloudinary Asset Storage**: Storefront photo / logo is streamed via `POST /api/v1/store/upload-asset` to Cloudinary folder `selfprint/stores/` and the secure URL is saved in MongoDB.
4. **Success & Auto-Redirect**: Shows 🎉 welcome card with generated Store ID, 4 feature activation cards, and automatically redirects to `/store/login` after a 5-second ticker.

### 9.2 Store Partner Login Flow
1. **Credentials**: Authenticates via `POST /api/v1/store/login` with Email or Phone and Password.
2. **MongoDB Lookup**: Queries `stores` collection with `.select('+password')`.
3. **Bcrypt Verification**: Verifies password against stored bcrypt hash (`passwordUtils.compare`).
4. **Account State Enforcement**: Validates that store `status === 'ACTIVE'`.
5. **JWT Token Issuance**: Signs JWT token with `STORE_OWNER` role and `storeId`.
6. **Dashboard Redirection**: Redirects to `/store/dashboard`, which immediately fetches live MongoDB data.

```mermaid
sequenceDiagram
    autonumber
    actor Store as Store Partner
    participant FE as Frontend Client (/store/login)
    participant API as Backend API (/api/v1/store/login)
    participant DB as MongoDB (stores)

    Store->>FE: Enter Email/Phone + Password
    FE->>API: POST /api/v1/store/login
    API->>DB: findOne({ email/phone }).select('+password')
    alt Store Not Found
        API-->>FE: 404 Not Found ("Store not found.")
    else Wrong Password
        API-->>FE: 401 Unauthorized ("Incorrect password.")
    else Inactive Store
        API-->>FE: 403 Forbidden ("Store account is inactive. Please contact Self Print Support.")
    else Valid Credentials
        API-->>FE: 200 OK (JWT Token + Store Profile)
        FE->>Store: Store JWT in localStorage & navigate to /store/dashboard
    end
```
...
### 10.1 Cloudinary vs Ephemeral Print Storage Policy
1. **Permanent Assets $\longrightarrow$ Cloudinary**:
   - Store logos, storefront photos, branding assets.
   - User profile avatars, Admin profile pictures.
   - Managed via [`cloudinaryService`](file:///d:/project/selfprint/backend/src/cloudinary/cloudinary.service.ts) using `.env` credentials (`CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_KEY_SECRET`).
2. **Temporary Customer Documents $\longrightarrow$ Local Ephemeral Disk Storage**:
   - Customer uploaded print documents (PDFs, DOCX, print images) are stored temporarily in `uploads/temp/` strictly during the active spooling cycle.
   - **Printing documents are NEVER stored permanently in Cloudinary**.
   - Temporary files are automatically purged from disk once the print job reaches a terminal state (`COMPLETED`, `FAILED`, `CANCELLED`).
...
sequenceDiagram
    ...
    participant TempStore as Local Ephemeral Storage
    participant DocSvc as Document Service

    User->>FE: Select / Drop document (PDF, DOCX, Image)
    FE->>API: POST /api/v1/user/upload (multipart/form-data)
    API->>TempStore: Save ephemeral file with UUID
    TempStore-->>API: Ephemeral file path
    API->>DocSvc: Calculate page count & color mode
    DocSvc-->>API: Total pages, dimensions, color info
    API-->>FE: Return Job Draft (id, pageCount, calculated price)
